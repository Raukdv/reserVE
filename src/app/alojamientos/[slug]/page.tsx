...
export default async function UnitPage({
  ...
  const amenityList = (amenities ?? [])
    .map((a) => a.amenities)
    .filter((a): a is { slug: string; label: string } => Boolean(a))
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        <Link href="/alojamientos" className="text-sm text-ink/50 hover:underline">
          ← Alojamientos
        </Link>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-4 sm:grid-rows-2">
          <UnitThumb slug={unit.slug} className="rounded-2xl sm:col-span-2 sm:row-span-2 sm:min-h-80" />
          <UnitThumb slug={`${unit.slug}-2`} className="min-h-32 rounded-2xl" />
          <UnitThumb slug={`${unit.slug}-3`} className="min-h-32 rounded-2xl" />
          <UnitThumb slug={`${unit.slug}-4`} className="min-h-32 rounded-2xl" />
          <UnitThumb slug={`${unit.slug}-5`} className="min-h-32 rounded-2xl" label="Sin fotos" />
        </div>

        <div className="mt-12 grid gap-12 lg:grid-cols-[1fr_380px]">
          <div>
            <p className="whitespace-pre-line leading-relaxed text-ink/75">{unit.description}</p>
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-ink/10 bg-white p-6 shadow-sm">
              <p className="text-2xl font-semibold">
                ...
                <span className="text-base font-normal text-ink/50"> / noche</span>
              </p>
              {unit.min_nights > 1 && (
              ...
              {quote?.ok && (
                <div className="mt-6 space-y-2 border-t border-ink/10 pt-5 text-sm">
                  ...
                  */}
                  ...
                  <div className="mt-4 rounded-xl bg-sand p-4 text-xs text-ink/65">
