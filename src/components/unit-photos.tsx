...
export function UnitPhotos({ unitId, photos }: { unitId: string; photos: UnitPhoto[] }) {
  ...
  async function onPick(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    // 1600 px es suficiente para pantalla completa sin cargar megas por foto.
    setFile(await compressImage(picked, { maxEdge: 1600, quality: 0.75 }))
    setWorking(false)
  }
  ...
  return (
    <section className="rounded-2xl border border-ink/10 bg-white p-6">
      ...
      <form onSubmit={onSubmit} className="mt-5 space-y-3">
        ...
        {working && <p className="text-xs text-ink/45">Optimizando…</p>}
...
function MoveButton({
  ...
  return (
    <form action={action}>
      ...
      <button
        disabled={pending}
        className="text-ink/45 hover:text-ink disabled:opacity-50"
